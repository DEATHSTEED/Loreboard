...
const LB_JUKEBOX_ERA_ASSETS = {
    '1920': 'modules/loreboard-lite/assets/graphics/BoardNoirmusik1920.webp',
    '1960': 'modules/loreboard-lite/assets/graphics/BoardNoirmusic1960.webp'
};
const LB_JUKEBOX_DEFAULT_ERA = '1920';

const LB_JUKEBOX_MODERN_DEVICE_ASSETS = {
    'cd': 'modules/loreboard-lite/assets/graphics/ModernOfficeCDPlayer.webp',
    'phone': 'modules/loreboard-lite/assets/graphics/ModernOfficeSmartphone.webp'
};
const LB_JUKEBOX_DEFAULT_MODERN_DEVICE = 'cd';

const LB_JUKEBOX_HIDEOUT_VIEW_ASSETS = {
    'device': 'modules/loreboard-lite/assets/graphics/ShadowpunkDevice.webp',
    'holo': 'modules/loreboard-lite/assets/graphics/ShadowpunkHolo.webp'
};
...
const LB_JUKEBOX_SLUMS_VIEW_ASSETS = {
    'mp3': 'modules/loreboard-lite/assets/graphics/SlumsMp3.webp',
    'phone': 'modules/loreboard-lite/assets/graphics/ModernOfficeSmartphone.webp',
    'device': 'modules/loreboard-lite/assets/graphics/ShadowpunkDevice.webp'
};
...
const LB_JUKEBOX_GALACTIC_VIEW_ASSETS = {
    'robot': 'modules/loreboard-lite/assets/graphics/GalaxyBackRobot.webp',
    'holo': 'modules/loreboard-lite/assets/graphics/ShadowpunkHolo.webp'
};
const LB_JUKEBOX_DEFAULT_GALACTIC_VIEW = 'robot';

const LB_JUKEBOX_COSYTAVERN_ASSET = 'modules/loreboard-lite/assets/graphics/CosyTavernclamp.webp';
...
function lbJukeboxInitBoard(html, store, theme, saveStore) {
    ...
    async function playRadioStation(station) {
        ...
        ui.notifications.warn('Radio konnte nicht verbunden werden. Kurz warten und erneut klicken.');
    }
...
async function lbSaveHiddenSnap(name, themeId, storeData) {
    ...
    try {
        ...
        ui.notifications.error('Board konnte nicht in Use Board gespeichert werden.');
        throw e;
    }
    return entry;
}
...
function lbJournalSelectHTML(selectedId, inputId) {
    ...
    return `<label class="theme-heading">Journal:</label>
        <select id="${inputId}-pick" class="lb-sec-journal-pick" style="width:100%;margin-bottom:6px;">
            <option value="">— Journal wählen —</option>${opts}
        </select>
...
function lbConfirmBoardLoadDialog(title, itemName, previewFn) {
    return new Promise(resolve => {
        new Dialog({
            title: title || 'Board laden?',
            content: `<div class="lb-modal-form"><p class="theme-heading" style="text-align:center;margin:0 0 10px 0;">Lade <b>${String(itemName || 'Auswahl').replace(/</g, '&lt;')}</b> — aktueller Stand wird überschrieben, sofern du nicht zuerst speicherst.</p><div id="lb-load-confirm-preview" style="width:240px;margin:0 auto;"></div></div>`,
            buttons: {
                load: { label: '<i class="fas fa-download"></i> Laden', callback: () => resolve('load') },
...
async function lbActivateBoard(themeId, store, boardName, opts) {
    ...
    if (game.user.isGM) {
        ...
        if (scene) {
            ...
            ui.notifications.warn('Keine aktive Szene — Board wird lokal geöffnet, Szene-Sync folgt beim nächsten Szenenwechsel.');
        }
        window.lbGmBoardLive = true;
        let syncPayload = lbSerializeBoardSyncState(store, { includePlayerRoles: true });
        try {
            game.socket.emit('module.loreboard-lite', {
...
async function lbCreateClassicBoard(classic, boardName) {
    ...
    let emptyStore = { items: [], threads: [], drawings: [], permissions: null, boardName: name, theme: 'noir', classic: true, classicBg: classic.bg, classicFg: classic.fg || (classic.customBoard ? LB_CUSTOM_BOARD_DEFAULT_FG_URL : ''), classicBoardId: classic.id, customBoard: !!classic.customBoard, jukeboxCustomView: classic.customBoard ? LB_CUSTOM_BOARD_DEFAULT_FG_VIEW : undefined };
...
function lbPromptBoardName(defaultName) {
    return new Promise((resolve) => {
        ...
        new Dialog({
            title: 'Board-Name',
            content: `<div class="lb-modal-form"><label class="theme-heading">Gib deinem Board einen Namen:</label><input type="text" id="lb-new-board-name" value="${safeDefault}" style="width:100%;" autocomplete="off" spellcheck="false"></div>`,
            buttons: {
                create: {
...
async function lbRunCreateBoardFromMenu(cat, classicEntry, themeObj, presetId, customClassic, defaultName) {
    ...
    try {
        if (cat === 'classic' && classicEntry) {
            ...
            ui.notifications.warn('Board-Typ konnte nicht ermittelt werden.');
            return false;
        }
        return true;
    } catch (e) {
        console.error('lbRunCreateBoardFromMenu', e);
        ui.notifications.error('Board konnte nicht erstellt werden.');
        return false;
    }
}
...
function lbParseConfig(j) {
    ...
    try {
        ...
        return c;
    } catch (e) { return { customBoards: [], stamps: [], tweakPresets: [], jukeboxLibrary: {} }; }
}
