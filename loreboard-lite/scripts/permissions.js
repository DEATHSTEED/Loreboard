...
function lbEmitBoardSync(kind, payload, opts) {
    ...
    try { game.socket.emit('module.loreboard-lite', msg); } catch (e) {}
}
...
window.lbBoardIsBusy = function() {
    return !!(document.querySelector('.lb-item.dragging') || window.lbIsDrawing || window.lbPlacingItem || window.lbPlacingCopy || window.lbIsDraggingPin || window.lbRevealPlaying || window.lbFlipAnimating);
};
function lbBindDocLockSocket() {
    if (window._lbDocLockSocketBound) return;
    window._lbDocLockSocketBound = true;
    game.socket.on('module.loreboard-lite', (data) => {
        if (!data) return;
