...
function lbBuildInspectDocumentFaceHTML(item, faceData, mirrored, faceSide) {
    ...
    if (bpFlag && bpModules.length) {
        ...
        if (cached) {
            blueprintLayers = lbWrapBlueprintBakedLayer(cached, faceData.blueprintBakedW || iw, faceData.blueprintBakedH || ih);
        } else {
    ...
    sharpenImages(root) {
        ...
        root.querySelectorAll('img').forEach((img) => {
            ...
            if (natW > 0 && boxW > 0 && boxW * (window.devicePixelRatio || 2) > natW * 0.92) {
                img.style.imageRendering = 'auto';
                img.style.filter = 'contrast(1.02) saturate(1.03)';
            }
        });
    },
...
function lbOpenCustomBoardUploadOverlay(opts) {
    ...
    let picked = { bg: existing ? null : '', name: existing ? existing.name : 'Custom Board' };
    ...
    let slotHTML = (which, label, val) => `
        <div class="lb-custom-board-slot" data-which="${which}">
            ...
            <div class="lb-custom-board-slot-prev" id="lb-cbu-${which}-prev" style="background:${val ? `url('${val}') center/cover no-repeat` : 'rgba(0,0,0,0.35)'};">${val ? '' : '<span>Kein Bild</span>'}</div>
            <div class="lb-custom-board-slot-actions">
                <button type="button" class="lb-ed-btn lb-cbu-pick" data-which="${which}"><i class="fas fa-folder-open"></i> Hochladen</button>
                <button type="button" class="lb-ed-btn lb-cbu-clear" data-which="${which}" title="Entfernen"><i class="fas fa-times"></i></button>
            </div>
        </div>`;
    let upload = $(`
        <div class="lb-custom-board-upload-overlay" id="lb-custom-board-upload">
            ...
            <div class="lb-custom-board-upload-panel lb-glass-surface">
                <div class="lb-custom-board-upload-head">
                    <span class="lb-custom-board-upload-title">${existing ? 'Custom Board bearbeiten' : 'Custom Board hochladen'}</span>
                ...
                <div class="lb-custom-board-upload-body">
                    ...
                    <p class="theme-heading lb-custom-board-hint">Eine einzelne Vollbild-Grafik für dein Custom Board.</p>
                </div>
                <div class="lb-custom-board-upload-foot">
                    <button type="button" id="lb-cbu-done" class="lb-custom-board-fertig-btn"><i class="fas fa-check"></i> Fertig</button>
                </div>
            </div>
        </div>`);
    $('body').append(upload);
    let closeUpload = () => {
        ...
        if (parentOv && parentOv.length) parentOv.removeClass('lb-create-overlay-suspended');
    };
    ...
    upload.find('.lb-cbu-pick').on('click', function() {
        ...
        lbOpenFilePicker({
            ...
            callback: (path) => {
                if (!path) return;
                picked[which] = path;
                let prev = upload.find('#lb-cbu-' + which + '-prev');
                prev.css('background', `url('${path}') center/cover no-repeat`).empty();
            }
        });
    });
    upload.find('.lb-cbu-clear').on('click', function() {
        let which = this.getAttribute('data-which');
        picked[which] = '';
        upload.find('#lb-cbu-' + which + '-prev').css('background', 'rgba(0,0,0,0.35)').html('<span>Kein Bild</span>');
    });
    upload.find('#lb-cbu-done').on('click', () => {
        picked.name = (upload.find('#lb-cbu-name').val() || 'Custom Board').trim();
        if (!existing && !picked.bg) return;
        closeUpload();
