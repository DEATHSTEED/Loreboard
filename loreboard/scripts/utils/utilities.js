...
function lbGetLanguageFontValue(langId, boardStore) {
    if (!langId) return '';
    boardStore = boardStore || (typeof store !== 'undefined' ? store : null) || window.lbCurrentStore || null;
...
function lbUserSpeaksLanguage(langId, userId) {
    ...
    // window.lbCurrentStore fallback: utilities has no lexical access to the board's
    // closure `store`, so without it players NEVER counted as speaking a language —
    // even when the GM granted it in the Foreign Language settings.
    let boardStore = (typeof store !== 'undefined' ? store : null) || window.lbCurrentStore || null;
...
function lbBuildDocEmbedsLayerHTML(item, faceSide, opts) {
    ...
    // z 15: effects/stamps are "printed on" the document — they must render in front of
    // ALL document content, including blueprint image/text modules whose baked host
    // wraps them at z-index 10 (which used to paint images over the embeds at z 5).
    return `<div class="lb-doc-embeds-layer" style="position:absolute;inset:0;pointer-events:none;overflow:hidden;z-index:15;">${parts.join('')}</div>`;
}
...
function lbGetAllBlueprints(store) {
    // Board-local blueprints merged with the world-level library (config journal):
    // saved blueprints must be loadable on ANY board and survive reloads.
    let out = [];
    let seen = {};
    let add = function(b) {
        if (!b || !b.id || lbIsDefaultBlueprintId(b.id) || seen[b.id]) return;
        seen[b.id] = true;
        out.push(foundry.utils.deepClone(b));
    };
    ((store && store.blueprints) || []).forEach(add);
    (window.lbGlobalBlueprints || []).forEach(add);
    return out;
}
...
function lbBuildBlueprintModulesHTML(modules, iw, ih, ctx) {
    ...
    return (modules || []).map(function(m) {
        ...
        if (m.type === 'text') {
            ...
            // NOTE: no language ciphering here — baked HTML is cached and SYNCED to all
            // clients, so the cipher must be applied per viewer at display time
            // (lbResolveBlueprintBakedLayer). Baking it froze the BAKING user's view
            // (usually the GM's readable text) into what every player saw.
...
function lbResolveBlueprintBakedLayer(item, faceData, bpModules, iw, ih) {
    // Language ciphering happens HERE, per viewer: the cached bake is language-neutral
    // and shared across clients — GM reads Handlee, players without the language skill
    // get the alien font, permitted players read normally.
    let docLang = (faceData && faceData.documentLanguage) || (item && item.documentLanguage) || '';
    let cached = faceData && faceData.blueprintBakedHtml;
    if (cached) {
        return lbWrapBlueprintBakedLayer(docLang ? lbMaybeCipherHtml(cached, docLang) : cached, faceData.blueprintBakedW || iw, faceData.blueprintBakedH || ih);
    }
    ...
    return lbWrapBlueprintBakedLayer(html && docLang ? lbMaybeCipherHtml(html, docLang) : html, iw, ih);
}
